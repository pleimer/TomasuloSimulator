			...
			for(int i=0;i<currInst.size();i++){ //+
				...
				switch(tok->getType()){
					...
					case REGISTER: {//registers
						...
						//reg.pop_back();
						reg = reg.substr(0, reg.size()-1);//+ pop back
						
						//unsigned int regNum = stoi(reg);
						unsigned int regNum; //+
						istringstream(reg) >> regNum;//+ string to int
					...
					case DECNUM: {//immediate
						if(tok->getCol() > 2){
							...
							//int num = stoi(strNum);
							int num; //+
							istringstream(strNum) >> num; //+ string to int
					...
					case BINUM:	{//immediate NOT WORKING
						if(tok->getCol() > 2){
							string strNum = tok->getCargo();
							strNum = strNum.substr(2,strNum.length());	
					...
					case REGOFFSET:{
						if(tok->getCol() > 1){
							...
							unsigned int regNum; //+
							istringstream(regStr.substr(1)) >> regNum;
							
							regNum = regNum << (INST_SIZE - OP_SIZE - REG_REF_SIZE*2);
							
							//immediate
							//num = stoi(numStr);
							istringstream(numStr) >> num; //+ string to int
...
vector<token> lexer::get(){
	...
	for(int i=0;i<token_strs.size();i++){ //+
	...
	if (isalpha(token_str[0]) != 0){
		...
		if (column == 0 && *token_str.rbegin() == ':') //+
			...
			if (token_str[0] == 'R'){
				...
				//int reg = stoi(regNum);
				int reg; //+
				istringstream(regNum) >> reg; //+ string to int
